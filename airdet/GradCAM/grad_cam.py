...
class GradCAM(object):
    ...
    def __call__(self, inputs, index=0):
        ...
        """
        self.net.zero_grad()
        output, proposal = self.net([inputs])
        print(output)
        ...
        cam /= np.max(cam)
        # resize to 224*224
        cpu_box = output[0]['instances'].pred_boxes.tensor[index].cpu().data
        pred_box = cpu_box.detach().numpy().astype(np.int32)
        cpu_box = proposal[0]['instances'].proposal_boxes.tensor[proposal_idx].cpu().data
        box = cpu_box.detach().numpy().astype(np.int32)
        x1, y1, x2, y2 = box
        cam = cv2.resize(cam, (x2 - x1, y2 - y1))

        cpu_id = output[0]['instances'].pred_classes[index].cpu().data
        class_id = cpu_id.detach().numpy()
        return cam, box, output[0]
