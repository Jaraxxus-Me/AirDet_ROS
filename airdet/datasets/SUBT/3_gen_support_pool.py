...
def main():
    basedir = './use'
    for d in tqdm(os.listdir(basedir)):
        print(d)
        dataDir = os.path.join(basedir, d)
        datasets = os.listdir(os.path.join(dataDir, 'new_annotations'))
        ...
        for dataType in datasets: #, 'train2017']:
            support_dict = {}
            ...
            set_crop_base_path = join(support_path, dataType[-6])
            set_img_base_path = join(dataDir, "JPEGImages")

            annFile = os.path.join(root_path, 'new_annotations/{}'.format(dataType))
            ...
            for img_id, id in enumerate(coco.imgs):
                ...
                support_df.to_pickle(os.path.join(dataDir, "./support_{}.pkl".format(dataType[-6])))
