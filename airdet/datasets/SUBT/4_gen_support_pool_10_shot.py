...
def main(test_path):
    ...
    support_path = os.path.join(dataDir, '3_shot_support')
    ...
    for dataType in ['JPEGImages']: #, 'train2017']:
        ...
        annFile = os.path.join(dataDir, 'new_annotations/new_split_subt_3_shot_instances_train.json')
...
if __name__ == '__main__':
    ...
    for test in tqdm(os.listdir(base_path)):
        if not (target_seq in test):
            continue
        test_path = os.path.join(base_path, test)
        support_df = main(test_path)
        support_df.to_pickle(os.path.join(test_path, "./3_shot_support_df.pkl"))
