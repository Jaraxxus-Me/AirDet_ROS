...
root_path = './use'
print(root_path)
for test in os.listdir(root_path):
    #root_path = '/home/fanqi/data/COCO'
    dataDir = os.path.join(root_path, test)
    ...
    for dataType in json_files: #, 'split_voc_instances_train2017.json']:
        ...
        inds_split2 = range(len(save_categories))
        ...
        dataset_split2 = {
            ...
            'categories': save_categories}
        new_annotations_path = os.path.join(dataDir, 'new_annotations')
        if not os.path.exists(new_annotations_path):
            os.makedirs(new_annotations_path)
        split2_file = os.path.join(dataDir, 'new_annotations/{}'.format(dataType))
