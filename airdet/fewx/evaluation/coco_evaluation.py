...
class COCOEvaluator(DatasetEvaluator):
    ...
    def _calculate_ar(self, class_names, recalls, M=None, A=None):
        ################## ar #####################
        voc_ls = []
        non_voc_ls = []
        ...
        for idx, name in enumerate(class_names):
            ...
            if M is not None and A is None:
                recall = recalls[:, idx, 0, M]
            elif A is not None and M is None:
                recall = recalls[:, idx, A, -1]
            ...
            # calculate voc ar and non-voc ar
            if name in CLASS_NAMES:
                voc_ls.append(ar * 100)
            else:
                non_voc_ls.append(ar * 100)
        if len(voc_ls) > 0:
            voc_ar = sum(voc_ls) * 1.0 / len(voc_ls)
        else:
            voc_ar = 0.0
        if len(non_voc_ls) > 0:
            non_voc_ar = sum(non_voc_ls) * 1.0 / len(non_voc_ls)
        else:
            non_voc_ar = 0.0

        return voc_ar, non_voc_ar
    ...
    def _derive_coco_results(self, coco_eval, iou_type, class_names=None):
        ...
        # calculate voc and non voc metrics for AP
        ...
        # calculate voc and non voc metrics for AR
        voc_ar, non_voc_ar = self._calculate_ar(class_names, recalls, M=0) # M=0, maxDets = 1
        voc_ar_10, non_voc_ar_10 = self._calculate_ar(class_names, recalls, M=1) # M=1, maxDets = 10
        voc_ar_100, non_voc_ar_100 = self._calculate_ar(class_names, recalls, M=2) # M=2 maxDets = 100

        voc_ar_small, non_voc_ar_small = self._calculate_ar(class_names, recalls, A=1) # A=1, small
        voc_ar_medium, non_voc_ar_medium = self._calculate_ar(class_names, recalls, A=2) # A=2, medium
        voc_ar_large, non_voc_ar_large = self._calculate_ar(class_names, recalls, A=3) # A=3, large

        # print voc ar
        self._logger.info("Evaluation results for VOC 20 categories =======> AR1  : " + str('%.2f' % voc_ar))
        self._logger.info("Evaluation results for VOC 20 categories =======> AR10: " + str('%.2f' % voc_ar_10))
        self._logger.info("Evaluation results for VOC 20 categories =======> AR100: " + str('%.2f' % voc_ar_100))
        self._logger.info("Evaluation results for VOC 20 categories =======> ARs : " + str('%.2f' % voc_ar_small))
        self._logger.info("Evaluation results for VOC 20 categories =======> ARm : " + str('%.2f' % voc_ar_medium))
        self._logger.info("Evaluation results for VOC 20 categories =======> ARl : " + str('%.2f' % voc_ar_large))

        # print voc ar
        self._logger.info("Evaluation results for Non VOC 60 categories =======> AR1  : " + str('%.2f' % non_voc_ar))
        self._logger.info("Evaluation results for Non VOC 60 categories =======> AR10: " + str('%.2f' % non_voc_ar_10))
        self._logger.info("Evaluation results for Non VOC 60 categories =======> AR100: " + str('%.2f' % non_voc_ar_100))
        self._logger.info("Evaluation results for Non VOC 60 categories =======> ARs : " + str('%.2f' % non_voc_ar_small))
        self._logger.info("Evaluation results for Non VOC 60 categories =======> ARm : " + str('%.2f' % non_voc_ar_medium))
        self._logger.info("Evaluation results for Non VOC 60 categories =======> ARl : " + str('%.2f' % non_voc_ar_large))
